...
// Fetch user info with contact_email
$stmt = $conn->prepare("SELECT id, username, full_name, gender, bio, tag, contact_email FROM users WHERE id = ?");
...
<title><?php echo htmlspecialchars($displayName); ?> - EditHub</title>
...
<style>
    ...
    .list-item { background: var(--canvas); border: 1px solid var(--line); border-radius: var(--radius); padding: 14px 18px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; gap: 12px; }
    .list-item h4 { font-size: 15px; font-family: var(--font-display); }
    .list-item p { font-size: 13px; color: var(--muted); margin-top: 2px; }
    .thumb-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 10px; }
    .thumb-card { background: var(--canvas); border: 1px solid var(--line); border-radius: var(--radius); overflow: hidden; color: #fff; display: block; }
    ...
    .btn-contact { background: #ff5232; color: #fff; padding: 10px 18px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 14px; display: inline-flex; align-items: center; gap: 6px; }
    .btn-contact:hover { background: #e04324; }
</style>
</head>
<body>
...
<div class="wrap">
    ...
    <div class="card profile-header">
        ...
        <?php if (!empty($contactEmail)): ?>
            <a href="mailto:<?php echo htmlspecialchars($contactEmail); ?>?subject=Inquiry%20from%20EditHub" class="btn-contact">
                ✉️ Contact / Hire Me
            </a>
        <?php endif; ?>
    </div>
    ...
    <div class="card card-pad" style="margin-bottom:24px;">
        <h3 style="margin-bottom:14px;">🎬 Services (Hire me)</h3>
        <?php if ($services->num_rows > 0): ?>
            <?php while ($s = $services->fetch_assoc()): ?>
                <div class="list-item">
                    ...
                    <?php if (!empty($contactEmail)): ?>
                        <a href="mailto:<?php echo htmlspecialchars($contactEmail); ?>?subject=Hire%20Inquiry:%20<?php echo urlencode($s['title']); ?>" class="btn-contact" style="padding: 6px 14px; font-size: 13px;">
                            Hire Service
                        </a>
                    <?php endif; ?>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="empty-msg">No services listed.</p>
        <?php endif; ?>
    </div>

    <div class="card card-pad" style="margin-bottom:24px;">
        <h3 style="margin-bottom:14px;">📦 Uploaded assets</h3>
        ...
        <?php else: ?>
            <p class="empty-msg">No assets uploaded.</p>
        <?php endif; ?>
    </div>

    <div class="card card-pad">
        <h3 style="margin-bottom:14px;">🖼️ Uploaded materials</h3>
        ...
        <?php else: ?>
            <p class="empty-msg">No materials uploaded.</p>
        <?php endif; ?>
    </div>
